...
<div class="tml tml-profile" id="theme-my-login<?php $template->the_instance(); ?>">
	...
	<form id="your-profile" action="<?php $template->the_action_url( 'profile', 'login_post' ); ?>" method="post">
		...
		<tr class="tml-nickname-wrap">
			<th><label for="nickname"><?php _e( 'ライターネーム', 'theme-my-login' ); ?> <span class="description"><?php _e( '(required)', 'theme-my-login' ); ?></span></label></th>
...
<table class="tml-form-table">
  ...
  <tr><small>※以下の項目は、記事下ライタープロフィールに表示されます</small></tr>
		...
		?>
		  ...
		  <h3><?php _e( "パスワードを新規作成  <small>※自動で生成されますが変更できます</small>", 'theme-my-login' ); ?></h3>
		<table class="tml-form-table">
		<tr id="password" class="user-pass1-wrap">
			<th><label for="pass1"><?php _e( 'New Password', 'theme-my-login' ); ?></label></th>
			<td>
				<input class="hidden" value=" " /><!-- #24364 workaround -->
				<button type="button" class="button button-secondary wp-generate-pw hide-if-no-js"><?php _e( '新しいパスワードを作成', 'theme-my-login' ); ?></button>
				<div class="wp-pwd hide-if-js">
					...
					<button type="button" class="button button-secondary wp-hide-pw hide-if-no-js" data-toggle="0" aria-label="<?php esc_attr_e( 'Hide password', 'theme-my-login' ); ?>">
						...
						<span class="text"><?php _e( '隠す', 'theme-my-login' ); ?></span>
					</button>
					<button type="button" class="button button-secondary wp-cancel-pw hide-if-no-js" data-toggle="0" aria-label="<?php esc_attr_e( 'Cancel password change', 'theme-my-login' ); ?>">
						<span class="text"><?php _e( 'キャンセル', 'theme-my-login' ); ?></span>
					</button>
				</div>
			</td>
		</tr>
		<tr class="user-pass2-wrap hide-if-js">
			...
			<p class="description"><?php _e( '新しいパスワードをもう一度入力して下さい', 'theme-my-login' ); ?></p>
			</td>
		</tr>
		<tr class="pw-weak">
			<th><?php _e( 'パスワード強度の確認', 'theme-my-login' ); ?></th>
			<td>
				<label>
					<input type="checkbox" name="pw_weak" class="pw-checkbox" />
					<?php _e( '簡単なパスワードであることを理解した上で使用します', 'theme-my-login' ); ?>
				</label>
			</td>
		</tr>
		<?php endif; ?>
